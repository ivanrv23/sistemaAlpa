    ...
    <admin-layout>
        ...
        <v-data-table :headers="headers" :items="desserts" sort-by="name" class="elevation-24" :search="search">
                                ...
                                <v-container>
                                    <v-row>
                                        ...
                                        <v-col cols="12" sm="6" md="6">
                                            <v-text-field v-model="editedItem.banking_entity" label="Entidad Bancaria"
                                                :rules="requiredField" required></v-text-field>
                                        </v-col>
                                        <v-col cols="12" sm="6" md="6">
                                            <v-text-field v-model="editedItem.account_number" label="Número de Cuenta"
                                                :rules="requiredField" required></v-text-field>
                                        </v-col>
                                        <v-col cols="12" sm="6" md="6">
                                            <v-text-field v-model="editedItem.cci" label="CCI"
                                                :rules="requiredField" required></v-text-field>
                                        </v-col>

                                        <v-col cols="12" sm="12" md="12">
                                            <v-textarea v-model="editedItem.description" class="mx-2"
                                                label="Descripción" rows="2" hint="Descripcion">
                                            </v-textarea>
                                        </v-col>
                                    </v-row>
                                </v-container>
            ...
            <template  v-slot:[`item.actions`]="{ item }">
                <v-icon v-if="can('Editar Cuentas Bancarias')" small class="mr-2" @click="editItem(item)">
                    mdi-pencil
                </v-icon>

                <v-icon v-if="can('Eliminar Cuentas Bancarias')" small @click="deleteItem(item)">
                    mdi-delete
                </v-icon>
            </template>
...
export default {
    ...
    data() {
        return {
            ...
            dialogDelete: false,
            headers: [
                { text: 'ENTIDAD', value: 'banking_entity' },
                { text: 'NRO CUENTA', value: 'account_number' },
                { text: 'CCI', value: 'cci' },
    ...
    methods: {
        ...
        initialize() {
            this.desserts = this.bankAccounts
        },
        ...
        deleteItemConfirm() {
            ...
            this.$inertia.delete(this.route('bankAccounts.destroy', this.editedItem))
            // ***************************************
        },
        ...
        save() {
            if (this.editedIndex > -1) {
                ...
                this.$inertia.patch(route('bankAccounts.update', this.editedItem), this.editedItem)
                // ***************************************
                ...
                // ***************************************
                // enviado formulario de almacenar 
                this.$inertia.post(route('bankAccounts.store'), this.editedItem)
