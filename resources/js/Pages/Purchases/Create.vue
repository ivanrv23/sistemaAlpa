  ...
  <admin-layout>
    ...
    <v-card class="overflow-hidden" color="white" elevation-10>
      <v-toolbar flat color="#ECEFF1">
        <v-toolbar-title class="font-weight-light"> Nueva Compra </v-toolbar-title>
      </v-toolbar>
      <v-card-text>
        <v-container>
          <v-row>
            ...
            <v-col cols="12">
              ...
              >
                ...
                <template v-slot:[`item.warehousesName`]="{ item }"
                  >{{ item.warehouses.name }}
                </template>
                <template v-slot:[`item.presentation`]="{ item }"
                  >{{ item.presentationName }}: {{ item.equivalence }} UND</template
                >
                <template v-slot:top>
                  <v-toolbar flat>
                          ...
                          <v-container>
                            <v-row>
                              ...
                              <v-col cols="12" sm="6" md="6">
                                <v-autocomplete
                                  ...
                                  :hint="
                                    ...
                                    editedItem.newMark == 0
                                      ? 'Stock: ' + editedItem.datosProducto.stock
                                      : 'Stock: 0'
                                  "
                                >
                                </v-autocomplete>
                              ...
                              <v-col
                                ...
                                md="3"
                                v-if="
                                  datosMoneda.code == 'USD' &&
                                  editedItem.igv == 'IGV Incluido' &&
                                  tipoCambio != '' &&
                                  editedItem.purchase_price != '' &&
                                  parseFloat(tipoCambio) > 0 &&
                                  editedItem.purchase_price > 0
                                "
                                v-show="true"
                              >
                                <v-text-field
                                  label="Precio S/."
                                  :value="
                                    editedItem.purchase_price * parseFloat(tipoCambio) +
                                    (editedItem.transporte * parseFloat(tipoCambio)) /
                                      editedItem.amount
                                  "
                                  type="number"
                                  readonly
                              ...
                              <v-col
                                ...
                                md="3"
                                v-if="
                                  datosMoneda.code == 'PEN' &&
                                  editedItem.igv == 'IGV Incluido' &&
                                  editedItem.transporte > 0 &&
                                  editedItem.purchase_price != '' &&
                                  editedItem.purchase_price > 0
                                "
                                v-show="true"
                              >
                                <v-text-field
                                  ...
                                  :value="
                                    parseFloat(editedItem.purchase_price) +
                                    editedItem.transporte / editedItem.amount
                                  "
                                  type="number"
                                  readonly
                                  outlined
                              ...
                              >
                                <v-text-field
                                  ...
                                  :value="
                                    parseFloat(editedItem.purchase_price) +
                                    editedItem.purchase_price * 0.18 +
                                    editedItem.transporte / editedItem.amount
                                  "
                                  type="number"
                                  readonly
                ...
                <template v-slot:no-data> No hay productos en el carrito </template>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="red" dark elevation="9" @click="limpiarCarrito"> Cancelar </v-btn>
        &nbsp;
        <template>
          <div class="text-center">
            <v-dialog v-model="dialogPago" width="450">
              ...
              <v-card>
                ...
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn color="red" text @click="closeDialoPago"> Cancelar </v-btn>
                  <v-btn color="primary" text @click="send_form"> Pagar </v-btn>
                </v-card-actions>
              </v-card>
            </v-dialog>
          </div>
...
export default {
  ...
  methods: {
    ...
    changeProduct() {
      ...
      this.editedItem.productMarksId = this.editedItem.datosProducto.marks_id;
      this.editedItem.productWarehousesId = this.editedItem.datosProducto.warehouses_id;
      ...
      this.totalSoles = 0;
      this.editedItem.warehouses = this.editedItem.datosProducto.warehouses[0];
    ...
    save() {
      if (this.editedIndex > -1) {
        ...
        if (this.editedItem.igv == "IGV Incluido") {
          var subTotal =
            this.editedItem.amount * this.editedItem.purchase_price +
        ...
        if (this.datosMoneda.code == "USD" && this.editedItem.igv == "IGV Incluido") {
          this.editedItem.precio_compra =
            ...
            (this.editedItem.transporte * this.tipoCambio) / this.editedItem.amount;
        }
        if (this.datosMoneda.code == "USD" && this.editedItem.igv == "Mas IGV") {
          this.editedItem.precio_compra =
            this.editedItem.purchase_price * this.tipoCambio +
            this.editedItem.purchase_price * this.tipoCambio * 0.18 +
