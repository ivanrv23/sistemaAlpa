<template>
  <admin-layout>
    ...
    class="elevation-24" :search="search" :footer-props="{ itemsPerPageText: 'Filas por página' }">
      <template v-slot:top>
        <v-toolbar flat>
      ...
      <template v-slot:[`item.actions`]="{ item }">
        <v-icon v-if="can('Editar Rol')" small class="mr-2" @click="editItem(item)">
          mdi-pencil
        </v-icon>
        
        <v-icon v-if="can('Eliminar Rol')" small @click="deleteItem(item)">
          mdi-delete
        </v-icon>
      </template>
...
export default {
  ...
  data() {
    return {
      ...
      { text: 'PERMISOS', value: 'cant_de_permisos' },
  ...
  methods: {
    ...
    deleteItemConfirm() {
      ...
      this.$inertia.delete(this.route('roles.destroy', this.editedItem))
      // ***************************************
    },
    ...
    save() {
      if (this.editedIndex > -1) {
        ...
        this.$inertia.patch(route('roles.update', this.editedItem), this.editedItem)
        // ***************************************
        ...
        // ***************************************
        // enviado formulario de almacenar 
        this.$inertia.post(route('roles.store'), this.editedItem)
